...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
* Puppetlabs no longer supports MCollective, so SIMP has removed the ``pupmod-simp-mcollective`` and
  ``pupmod-simp-activemq`` modules that supported MCollective.
...
* This RPM has been removed, as it is no longer a dependency of any SIMP tools.
